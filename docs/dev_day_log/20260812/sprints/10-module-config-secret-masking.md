...
| `sMollieProfileId` | **no** | `pfl_…` is shipped to the browser by the OPC footer widget for `new Mollie(profileId, …)`. It is public by design; masking it in admin would imply a confidentiality it does not have |
| `sMollieWebhookUrl` | **no** | A public URL. Mollie sends no signature (see `CLAUDE.md` — verification is an API re-fetch), so there is no webhook secret to protect |
...
Net effect: this sprint adds **zero** PHP to `src/`. Only a template, two lang idents, and tests.
...
`stripe-key-*` names: three modules ship a `module_config` override in this shop, and clashing
selectors would let one module's inline JS bind to another's buttons.

### D4 — Block delegation is mandatory
...
form. Story 4 verifies this against all three PSP modules active simultaneously.
...
### Story 4: Three-PSP coexistence verification

**As a** shop operator running Mollie next to Stripe and PayPal
**I want** every module's settings tab to keep working
**So that** the new override does not blank out another module's form

Stripe and PayPal each already ship `admin_twig/module_config.html.twig`. Mollie makes three
overrides of the same template in one shop — this is the real risk in the sprint, and it is
integration-level, not unit-level.

**Acceptance Criteria:**
- [ ] Integration test (or documented manual run, if the module-template chain cannot be built
      in the Integration suite): with `oe_payments_mollie`, `osc_stripe_wallet` and
      `oe_payments_paypal` all active, the resolved `module_config` template chain includes all
      three overrides and rendering does not throw
- [ ] Manual verification, recorded in `done/`: open **each** of the three modules' Settings
      tabs and confirm
...
**Files:**
- `tests/Integration/Module/ModuleConfigTemplateChainTest.php` — new (if feasible)
...
- [ ] Story 4 three-module verification recorded in `done/`
...
|---|---|---|
| Mollie's block swallows Stripe's/PayPal's fields via the shared template chain | medium | D4 `{{ parent() }}` delegation + `getEditObjectId()` gate + Story 4 |
