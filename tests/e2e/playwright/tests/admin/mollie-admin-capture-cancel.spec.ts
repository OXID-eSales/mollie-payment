 ...
 *
 * The target orders are injected by ORDER NUMBER via env. Each must have a Mollie payment in
 * status `authorized` (an uncaptured hold). The contract may be `committed` — a normal
 * manual-capture checkout lands there and the panel still offers the buttons, gating on the
 * live Mollie status rather than contract state:
...
test.describe('Mollie admin: capture & cancel authorization', () => {
    test('Capture: partial capture of an authorized payment', async ({ page }) => {
        ...
        // amount now reflects the partial capture. (The capture card's disappearance depends on
        // Mollie propagating the payment to `paid`, which is eventually consistent, so it is
        // logged rather than hard-asserted here.)
        await orders.openPaymentTab();
        ...
        console.log(`  capture card still shown (Mollie propagation): ${await mollie.isCaptureButtonVisible()}`);
    });
